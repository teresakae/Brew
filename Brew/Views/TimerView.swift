...
// ScrollView, LazyHStack, scrollTargetBehavior, containerRelativeFrame
...
// MARK: - Circle Control Button (Native)
...
// MARK: - Timer View (Root Screen)

struct TimerView: View {

    @State private var viewModel = BrewTimerViewModel(phases: BrewPhase.frenchPressSample)
    
    // hard coded until recipe model is ready
    let recipeName  = "French Press"
    let coffeeGrams = 25
    let waterMl     = 250
    let totalTime   = "2:30"
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                VStack(spacing: 0) {
                    Text("\(coffeeGrams)g · \(waterMl)ml · \(totalTime)")
                        .font(.subheadline)
